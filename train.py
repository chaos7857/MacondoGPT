...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(filename=f'logs/{current_time}.log', encoding='utf-8')
    ]
)
logging.debug("日志配置完毕")
logging.debug("="*60)
#############################################################################################
...
def get_batch(split):
    ...
    if split == 'train':
        # 将文件内容映射到虚拟内存空间，程序可以像访问普通数组一样访问文件数据，但实际数据仍在磁盘上。
...
while True:
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        if local_iter_num >= 5: # let the training loop settle a bit  让训练循环先稳定运行一段时间，再进行关键操作（如评估、调整超参数等）
            # mfu = raw_model.estimate_mfu(batch_size * gradient_accumulation_steps, dt)# 模型利用率
            mfu = raw_model.estimate_mfu(batch_size * gradient_accumulation_steps, dt)# 模型利用率
            running_mfu = mfu if running_mfu == -1.0 else 0.9*running_mfu + 0.1*mfu # 平滑计算模型利用率
        ...
        # if local_iter_num >= 5: # let the training loop settle a bit  让训练循环先稳定运行一段时间，再进行关键操作（如评估、调整超参数等）
        #     # mfu = raw_model.estimate_mfu(batch_size * gradient_accumulation_steps, dt)# 模型利用率
        #     mfu = raw_model.my_estimate_mfu(batch_size * gradient_accumulation_steps, dt, batch_size)# 模型利用率
        #     running_mfu = mfu['compute_mfu'] if running_mfu == -1.0 else 0.9*running_mfu + 0.1*mfu['compute_mfu'] # 平滑计算模型利用率
        # logging.info(f"iter {iter_num}: loss {lossf:.4f}, time {dt*1000:.2f}ms, mfu {running_mfu*100:.2f}%， memory_mfu {mfu['memory_mfu']*100:.2f}%, bottleneck: {mfu['bottleneck']}")
        #####################################################
    iter_num += 1
    local_iter_num += 1
